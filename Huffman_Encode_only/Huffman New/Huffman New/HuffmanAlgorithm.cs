...
namespace Huffman_New
{
    ...
    public class HuffmanAlgorithm : IDisposable
    {
        ...
        public Int32 ShrinkWithProgress(Stream Data, string OutputFile)
        {
            ...
            Int32 s= 0;
            short j; int k=0; float ProgressRatio = 0;
        ...
        public float GetFileSize(Stream Data)
        {
            Data.Seek(0, SeekOrigin.Begin);
            float Result=0;
            try
            {
                FileHeader Header = (FileHeader)BinFormat.Deserialize(Data);
                Result = Header.OriginalSize;
                Header = null;
            }
            catch (Exception)
            {
                //if header wasn't found
                MessageBox.Show("Error, the given stream isn't Huffman archived or corrupted.");
            }
            finally
            {
                Data.Seek(0, SeekOrigin.Begin);
        ...
        private FrequencyTable BuildFrequencyTable(Stream DataSource)
        {
            ...
            for (long i = 0; i < DataSource.Length; ++i)
            {
                ...
                if (((float)(i)) / 10000 - ProgressRatio > 0.01)
                {
                    ProgressRatio += 0.01f;
                    if (OnPercentCompleted != null) OnPercentCompleted();
                }
        ...
        public PercentCompletedEventHandler PercentCompleted { get; set; }
    }
}
